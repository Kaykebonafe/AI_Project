...
static int maintain_leds_on(int numMaintainOn){
    if(numMaintainOn == 0)
        return 0;
    
    for (int i = 0; i < numMaintainOn; i++) {
      leds_values[i] = true;
    
    }
    return numMaintainOn;

}

static void restart_leds() {
  aux_leds_verify = 0;
...
int main(int argc, char **argv) {
  ...
  int aux = 0;
  int times_blinked = 0;
  bool blink_times_vec[3] = {false, false, false};
  bool touched = false;
  bool touching = false;

  while (wb_robot_step(TIME_STEP) != -1) {
    reset_actuator_values();
    aux = maintain_leds_on(aux);
    get_sensor_input();
    if ((get_sensor_input()) && (wb_motor_get_velocity(left_motor) > 2.2) && (wb_motor_get_velocity(left_motor) < 2.35) || (wb_motor_get_velocity(right_motor) > 2.2) && (wb_motor_get_velocity(right_motor) < 2.8) && aux_leds_verify < 9){
      touched = true;
    } else {
      touching = false;
    }
    if(touched == true){
      if(aux_leds_verify < 9 && blink_times_vec[2] == false){
        aux_leds_verify++;
        touching = true;
      } else if(!touching && blink_times_vec[2] == true){
        blink_times_vec[0] = false;
        blink_times_vec[1] = false;
        blink_times_vec[2] = false;
        touched = false;
        if(aux <= 9)
          aux = maintain_leds_on(aux+1);
        times_blinked = 0;
      }
          blink_leds();
    }
    if(aux_leds_verify >= 9){
      blink_times_vec[times_blinked] = true;
      times_blinked++;
      restart_leds();
    }
    if (cliff_detected()) {
